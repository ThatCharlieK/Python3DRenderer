...
class Renderer:
    ...
    def draw_point(self, point: Vector3) -> None:
        ...
        if projected:
            pygame.draw.circle(self.screen, constants.RED, projected, 5)
    ...
    def render(self, points: List[Vector3], edges: List[Tuple[int, int]]) -> None:
        self.screen.fill(constants.WHITE)
        for edge in edges:
            self.draw_line(points[edge[0]], points[edge[1]])
        for point in points:
...
if __name__ == "__main__":
    ...
    points, edges = generate_geometry.generate_cube(Vector3(10, 10, 10), 20)
    running = True
    while running:
        renderer.render(points, edges)
